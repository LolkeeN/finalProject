        ...
        try {
            ...
            for (Meeting meeting:meetings) {
                ...
                while(resultSet.next()){
                    Location location = locationDao.getLocation(rs.getInt("location_id"));
                    meeting.setLocation(location);
                }
            }
        }catch (SQLException e){
            log.error("Cannot get all meetings", e);
            throw new IllegalArgumentException("Cannot get all meetings", e);
        }
        return meetings;
    }

    public void setMeetingDate(String name, String date){
        Meeting meeting = new Meeting();
        meeting.setName(name);
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
