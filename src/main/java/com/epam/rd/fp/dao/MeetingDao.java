...
public class MeetingDao {
    ...
    public void insertMeeting(Meeting meeting) {
        ...
        if (date.getTime() > (meetingDate.getTime())) {
            log.warn("Cannot create meeting with the date that has passed");
     ...
     *
     * @param meetingId   id of meeting which date you want to change
    ...
    public void setMeetingDate(int meetingId, String date) {
        ...
        if (date1.getTime() != (meetingDate.getTime())) {
            log.warn("Cannot update meeting date with the date that has passed");
